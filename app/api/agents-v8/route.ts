...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        if (backend === 'iii') {
          await runIiiBackend({ mode: 'v8', message, model: resolvedModel, providerId: creds.providerId, apiKey: creds.apiKey, history: priorTurns, conversationId: parsed.body.conversationId, send });
        } else {
