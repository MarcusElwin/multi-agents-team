...
export async function POST(req: NextRequest) {
  ...
  console.log('\n========================================');
  console.log(`🎬 API REQUEST: v9 swarm · model=${resolvedModel}`);
  console.log('========================================\n');
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        if (backend === 'iii') {
          await runIiiBackend({ mode: 'v9', message, model: resolvedModel, providerId: creds.providerId, apiKey: creds.apiKey, history: priorTurns, send });
        } else {
        ...
        console.log('\n========================================');
        console.log('✅ API RESPONSE: v9 Workflow Complete');
        console.log('========================================\n');
      } catch (error) {
        const errMsg = describeError(error);
        console.error('❌ v9 API Error:', errMsg);
...
export async function GET() {
  return NextResponse.json({
    status: 'ready',
    pattern: 'swarm (identical agents leave traces; emergent convergence)',
  });
}
