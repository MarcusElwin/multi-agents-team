...
export async function POST(req: NextRequest) {
  ...
  console.log('\n========================================');
  console.log(`🎬 API REQUEST: v5 debate · model=${resolvedModel}`);
  console.log('========================================\n');
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        if (backend === 'iii') {
          await runIiiBackend({ mode: 'v5', message, model: resolvedModel, providerId: creds.providerId, apiKey: creds.apiKey, history: priorTurns, conversationId: parsed.body.conversationId, send });
        } else {
          const conversation = new Conversation(priorTurns);
          await runDebate(message, { model: resolvedModel, apiKey: creds.apiKey, providerId: creds.providerId }, send, conversation);
        }
        // runDebate emits its own workflow_complete.

        console.log('\n========================================');
        console.log('✅ API RESPONSE: v5 Workflow Complete');
        console.log('========================================\n');
      } catch (error) {
        const errMsg = describeError(error);
        console.error('❌ v5 API Error:', errMsg);
...
export async function GET() {
  return NextResponse.json({
    status: 'ready',
    pattern: 'debate / consensus (opposing stances → judge)',
  });
}
